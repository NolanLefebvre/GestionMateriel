...
namespace LyonPalme
{
    ...
    public class DBInterface
    {
        ...
        {
            //creation d'une list de materiel vide
            ...
            try
            {
                ...
                using (SqlCommand sqlCommand = new SqlCommand("GM_spMaterielStock", connection))
                {
                    ...
                    {
                        //boucle qui se répéte le nombre de fois que on recois de materiel
                        while (sqlDataReader.Read())
                        {
                            //remplissage d'un materiel modele
                            ...
                            //ajout du materiel dans le stock
            ...
            //retourne la liste de materiel remplie
            return stocks;
        }
        ...
        //récupération des variable a ajouter pour le préts
        public static void AddMateriel(string marque, string libelle, string etat ,string taille, int? pointure)
        {
            ...
            try 
            {
                ...
                using (SqlCommand sqlCommand = new SqlCommand("GM_spAddMaterielStock", connection))
                {
                    ...
                    //verifie si il y a une taille qui a était envoyer dans la fonction (string)
                    if (taille != null)
                    {
                        sqlCommand.Parameters.AddWithValue("@taille", taille);
                    }
                    //et sinon rentre NULL dans la procédure
                    else
                    {
                        sqlCommand.Parameters.AddWithValue("@taille", DBNull.Value);
                    }
                    //verifie si il y a une pointure qui a était envoyer dans la fonction (int) 
